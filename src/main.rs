...
fn main() {
    ...
    let printf = ctx.new_function(None,FunctionType::Extern,ctx.new_type::<()>(),&[ctx.new_parameter(None,ptr_ty,"fmt")],"printf",true);
    let block = add2.new_block("entry");

    let param = add2.new_local(None,int,"n");

    block.add_assignment(None,param,add2.get_param(0).to_rvalue() + ctx.new_rvalue_from_int(int,4));
    block.add_eval(None,ctx.new_call(None,printf,&[ctx.new_string_literal("Value %i\n") ,param.to_rvalue()]));
